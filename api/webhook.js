...
// ── Dedicated-number lookup (FIXED) ───────────────────────────────────────
// Uses `dedicated_whatsapp_number` — a NEW column, separate from the
// pre-existing `whatsapp_number` column (which already stores each salon's
// own contact/display WhatsApp number and must not be touched). Returns
// null if no salon has a dedicated number configured, which safely falls
// through to the existing keyword/name matching below.
async function getSalonByNumber(toNumber) {
  if (!toNumber) return null;
  try {
    const digits = toNumber.replace(/\D/g, "");
    if (!digits) return null;
    const r = await fetch(`${SUPABASE_URL}/rest/v1/salons?dedicated_whatsapp_number=eq.${digits}&limit=1`, { headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` } });
    const d = await r.json();
    return d?.[0] || null;
...
export default async function handler(req, res) {
  ...
  // The business number this message arrived on (for dedicated-number
  ...
  try {
    ...
    if (pendingPhotos?.data?.photos?.length) {
      ...
      const pCreds = pSalon ? { apiKey: pSalon.ycloud_api_key || YCLOUD_KEY, botNumber: pSalon.dedicated_whatsapp_number || BOT_NUMBER } : null;
    ...
    // (dedicated_whatsapp_number), otherwise the shared global env vars
    // (unchanged behaviour for every existing shared-number salon).
    const creds = { apiKey: salon.ycloud_api_key || YCLOUD_KEY, botNumber: salon.dedicated_whatsapp_number || BOT_NUMBER };
