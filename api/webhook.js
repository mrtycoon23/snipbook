...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (step === "ask_name") {
      ...
      if (rows.length > 0) {
        await sendList(from, "💇 Service Chunein", `Nice to meet you, *${text}!* 🙌\n\nKonsi service chahiye?`, "Service Dekho", rows);
      } else {
        await sendButtons(from, `Nice to meet you, *${text}!* 🙌\n\nKonsi service?`, [
          { id: "svc_1", title: "✂️ Haircut ₹250" },
          { id: "svc_2", title: "✂️ Haircut+Beard ₹450" },
          { id: "svc_3", title: "🎨 Hair Colour ₹1200" },
        ]);
      }
      res.status(200).json({ status: "ok" });
      return;
    ...
    if (step === "confirm" && interactiveId === "confirm_yes") {
      await fetch(`${SUPABASE_URL}/rest/v1/appointments`, {
        ...
        body: JSON.stringify({
          salon_id: SALON_ID, service: data.service, amount: data.price,
          date: data.date, time_slot: data.time, status: "confirmed",
        }),
      });
      await clearSession(from);
      const successMsg =
