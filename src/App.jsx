...
function LoginPage({onOwnerLogin,onStaffLogin,onSignup,onBack}){
  ...
  if(!salonCheck||salonCheck.length===0){
    alert("⚠️ Is email se koi account nahi mila. Signup karte waqt jo email use ki thi wahi daalo.");
    return;
  }
  const{error}=await supabase.auth.resetPasswordForEmail(emailLower,{redirectTo:"https://snipbook.vercel.app"});
  if(error)alert("Error: "+error.message);
  else alert("✅ Password reset link bhej diya! Email check karo.");
...
function Onboarding({onComplete,onBack}){
  ...
  const F=OBF;
  async function handleComplete(){setLoading(true);setError("");try{const{data:authData,error:authErr}=await supabase.auth.signUp({email:data.email,password:data.password});if(authErr)throw authErr;if(!authData?.user?.id){setError("Yeh email already registered hai. Kripya doosra email use karein ya support se contact karein.");setLoading(false);return;}const userId=authData.user.id;const botKeyword=slugify(data.salonName);const{error:salonErr}=await supabase.from("salons").insert({id:userId,owner_name:data.ownerName,salon_name:data.salonName,phone:data.phone,city:data.city,plan:"free",services:svcs.filter(s=>s.active),working_days:data.workDays,open_time:data.openTime,close_time:data.closeTime,whatsapp_number:data.waNumber,bot_keyword:botKeyword,notification_number:data.phone,notification_email:data.email});if(salonErr){setError(`Salon save karne mein error: ${salonErr.message}`);setLoading(false);return;}onComplete({id:userId,name:data.ownerName,email:data.email,salon:data.salonName,city:data.city,plan:"Trial",logo_url:null});}catch(e){setError(e.message);setLoading(false);}} 
...
function ChangePasswordSection(){
  ...
  async function handleChange(){
    if(newPass.length<6){setStatus("error:Min 6 characters chahiye");return;}
    if(newPass!==confirm){setStatus("error:Dono passwords match nahi karte");return;}
    ...
    <div style={{background:"#fff",borderRadius:14,border:"1px solid #f1f0f5",overflow:"hidden",marginBottom:10}}>
      <div onClick={()=>setOpen(o=>!o)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 16px",cursor:"pointer"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{fontWeight:700,fontSize:15,color:"#0f0a2e"}}>Password Change Karo</div>
        </div>
        <span style={{color:"#9ca3af",fontSize:14}}>{open?"▲":"▼"}</span>
      </div>
      {open&&<div style={{padding:"0 16px 16px",display:"flex",flexDirection:"column",gap:10}}>
        {status==="success"&&<div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:10,padding:"10px 12px",fontSize:13,color:"#16a34a",fontWeight:700}}>✅ Password change ho gaya!</div>}
        ...
        <div>
          <div style={{fontSize:12,fontWeight:600,color:"#6b7280",marginBottom:5}}>Naya Password</div>
        ...
        <div>
          <div style={{fontSize:12,fontWeight:600,color:"#6b7280",marginBottom:5}}>Password Confirm Karo</div>
          <input type="password" value={confirm} onChange={e=>setConfirm(e.target.value)} placeholder="Same password dobara" style={inp2} onFocus={e=>e.target.style.borderColor="#5b3fc4"} onBlur={e=>e.target.style.borderColor="#e5e7eb"}/>
        </div>
...
function Settings({user,onBack,onLogout,onSalonUpdate,showRevenue,setShowRevenue}){
  ...
  async function save(){
    if((profile.phone&&profile.phone.length<10)||(profile.notifNumber&&profile.notifNumber.length<10)){
      setSaveError("Phone aur Notification Number — dono minimum 10 digits ke hone chahiye");
  ...
  return(
    <div style={{height:"100dvh",display:"flex",flexDirection:"column",fontFamily:"system-ui,sans-serif",color:TP.text,background:TP.bg,overflow:"hidden"}}>
      ...
      {bookingToast&&(<div onClick={()=>{setScreen("calendar");if(bookingToast.date)setSelDate(new Date(bookingToast.date));setBookingToast(null);}} style={{position:"fixed",top:"calc(12px + env(safe-area-inset-top,0px))",left:12,right:12,zIndex:999,background:"linear-gradient(135deg,#16a34a,#22c55e)",borderRadius:16,padding:"14px 16px",boxShadow:"0 8px 24px rgba(0,0,0,0.3)",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontWeight:800,fontSize:14,color:"#fff"}}>Nayi Booking Aayi!</div>
