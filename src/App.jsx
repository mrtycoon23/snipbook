  ...
  return(<div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:"#f8f7ff"}}>
    ...
    <div style={{flex:1,overflowY:"auto",padding:"14px 14px 80px"}}>
      {tab==="profile"&&(<div style={{flex:1,overflowY:"auto",paddingBottom:80}}>
        ...
        <div style={{background:"#fff",borderRadius:14,margin:"10px 16px 0",border:"1px solid #f1f0f5",overflow:"hidden"}}>{sectionTitle("🏪","Salon Details")}<Divider/><div style={{padding:"14px 16px",display:"flex",flexDirection:"column",gap:12}}>{[{label:"Salon Name",key:"salonName",placeholder:"e.g. Style Adda"},{label:"Owner Name",key:"ownerName",placeholder:"e.g. Rahul Sharma"},{label:"Phone Number",key:"phone",placeholder:"9876543210"},{label:"City",key:"city",placeholder:"e.g. Delhi"},{label:"Address",key:"address",placeholder:"e.g. Shop 12, MG Road"},{label:"Google Maps Link",key:"mapsLink",placeholder:"https://maps.google.com/..."},{label:"Notification Number",key:"notifNumber",placeholder:"9876543210"}].map(f=>{const isPhoneField=f.key==="phone"||f.key==="notifNumber";const val=profile[f.key]||"";const tooShort=isPhoneField&&val.length>0&&val.length<10;return(<div key={f.key}><FieldLabel>{f.label}</FieldLabel><input value={val} onChange={e=>setProfile(p=>({...p,[f.key]:isPhoneField?e.target.value.replace(/\D/g,"").slice(0,10):e.target.value}))} placeholder={f.placeholder} maxLength={isPhoneField?10:undefined} inputMode={isPhoneField?"numeric":undefined} style={inp} onFocus={e=>e.target.style.borderColor="#5b3fc4"} onBlur={e=>e.target.style.borderColor="#e5e7eb"}/>{isPhoneField&&<div style={{fontSize:10,color:tooShort?"#dc2626":"#9ca3af",marginTop:4,fontWeight:tooShort?700:500}}>{tooShort?`⚠️ Abhi sirf ${val.length} digits hain — `:""}Minimum 10 digits required</div>}</div>);})}</div></div>
