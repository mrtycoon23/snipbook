    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#f0fdf4,#f0f4f8)",display:"flex",flexDirection:"column",alignItems:"center",padding:"20px 16px 40px",fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{width:"100%",maxWidth:460,background:"#fff",borderRadius:20,boxShadow:"0 8px 28px rgba(0,0,0,0.08)",border:"2px solid #e8edf3",overflow:"hidden"}}>
        ...
        <div style={{padding:"16px 20px"}}>
          {error&&<div style={{background:"#fff0f0",border:"2px solid #fca5a5",borderRadius:10,padding:"9px 12px",fontSize:12,color:"#dc2626",fontWeight:700,marginBottom:14}}>{error}</div>}
          {step===1&&(<><F label="Full Name"><input value={data.ownerName} onChange={e=>setData(p=>({...p,ownerName:e.target.value}))} placeholder="e.g. Rahul Sharma" style={obIs}/></F><F label="Mobile"><input value={data.phone} onChange={e=>setData(p=>({...p,phone:e.target.value.replace(/\D/g,"").slice(0,10)}))} maxLength={10} inputMode="numeric" placeholder="+91 98765 43210" style={obIs}/></F><F label="Email"><input value={data.email} onChange={e=>setData(p=>({...p,email:e.target.value}))} placeholder="rahul@gmail.com" type="email" style={obIs}/></F><F label="Password"><input value={data.password} onChange={e=>setData(p=>({...p,password:e.target.value}))} placeholder="Min 6 characters" type="password" style={obIs}/></F></>)}
  ...
  async function save(){
    ...
    try{
      await supabase.from("salons").update({
        ...
        notification_email: profile.notifEmail || "",
      }).eq("id", user.id);
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px 14px 80px"}}>
        {tab==="profile"&&(<>
          ...
          <Card title="Salon Details" icon="🏪">
            ...
            <F label="Notification Email" hint="Booking alerts is email pe aayenge"><input value={profile.notifEmail||""} onChange={e=>setProfile(p=>({...p,notifEmail:e.target.value}))} placeholder="e.g. owner@gmail.com" type="email" style={inputStyle}/></F>
        ...
        {screen==="dashboard"&&(
          <div style={{padding:"14px 16px",overflowY:"auto",flex:1}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
              {[{icon:"📅",val:booked,label:"Bookings",bg:"#e8fdf0",color:"#16a34a"},{icon:"💸",val:`₹${revenue.toLocaleString()}`,label:"Revenue",bg:"#e8fdf0",color:"#16a34a"},{icon:"⏳",val:pending,label:"Pending",bg:"#fff8e6",color:"#d97706"},{icon:"💬",val:"Chats",label:"Bot History",bg:"#eff6ff",color:"#2563eb",click:true}].map(s=>(<div key={s.label} onClick={()=>{if(s.click)setScreen("chats");}} style={{background:s.bg,borderRadius:14,padding:"14px",border:`2px solid ${s.color}22`}}><div style={{fontSize:20,marginBottom:5}}>{s.icon}</div><div style={{fontWeight:900,fontSize:s.click?16:22,color:s.color,lineHeight:1}}>{s.val}</div><div style={{fontSize:11,color:"#888",fontWeight:700,marginTop:3}}>{s.label}</div></div>))}
            </div>
            <div style={{marginBottom:14}}>
            ...
            {showEditClient&&editClient&&(
              <div onClick={()=>setShowEditClient(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:600,display:"flex",alignItems:"flex-end"}}>
                <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"20px 20px 0 0",padding:"18px 18px 32px",width:"100%",maxHeight:"90vh",overflowY:"auto"}}>
                  ...
                  {[{label:"Full Name *",key:"name",ph:"e.g. Priya Sharma",type:"text"},{label:"Phone Number",key:"phone",ph:"9876543210",type:"tel",maxLength:10},{label:"City",key:"city",ph:"Delhi",type:"text"},{label:"Date of Birth",key:"dob",ph:"",type:"date"} ,{label:"Email Address",key:"email",ph:"customer@gmail.com",type:"email"} ].map(f=>(<div key={f.key} style={{marginBottom:12}}><div style={{fontSize:13,fontWeight:800,color:"#555",marginBottom:4}}>{f.label}</div><input type={f.type} value={editClient[f.key]||""} onChange={e=>setEditClient(p=>({...p,[f.key]:e.target.value}))} placeholder={f.ph} style={{...is}} onFocus={e=>e.target.style.borderColor="#22c55e"} onBlur={e=>e.target.style.borderColor="#e8edf3"}/></div>))}
                  ...
                  <div style={{display:"flex",gap:10}}>
                    ...
                    <button onClick={async()=>{
                      ...
                      await supabase.from("customers").update({name:editClient.name.trim(),phone:editClient.phone||"",city:editClient.city||"",birthday:editClient.dob||null,email:editClient.email||"",tag:editClient.tag||"Regular",gender:editClient.gender||"male"}).eq("id",editClient.id);
            ...
            {showAddClient&&(
              <div onClick={()=>setShowAddClient(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:600,display:"flex",alignItems:"flex-end"}}>
                <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"20px 20px 0 0",padding:"18px 18px 32px",width:"100%",maxHeight:"90vh",overflowY:"auto"}}>
                  ...
                  {[{label:"Full Name *",key:"name",ph:"e.g. Priya Sharma",type:"text"},{label:"Phone Number *",key:"phone",ph:"9876543210",type:"tel",maxLength:10},{label:"City",key:"city",ph:"Delhi",type:"text"},{label:"Date of Birth",key:"dob",ph:"",type:"date"}].map(f=>(<div key={f.key} style={{marginBottom:12}}><div style={{fontSize:13,fontWeight:800,color:"#555",marginBottom:4}}>{f.label}</div><input type={f.type} value={newClient[f.key]} onChange={e=>setNewClient(p=>({...p,[f.key]:f.key==="phone"?e.target.value.replace(/\D/g,"").slice(0,10):e.target.value}))} placeholder={f.ph} maxLength={f.key==="phone"?10:undefined} inputMode={f.key==="phone"?"numeric":undefined} style={{...is}} onFocus={e=>e.target.style.borderColor="#22c55e"} onBlur={e=>e.target.style.borderColor="#e8edf3"}/></div>))}
