...
// ─── Pending Modal ────────────────────────────────────────────────────────────
function PendingModal({salonId, staffMap, onClose, onUpdate}){
  ...
  return(
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:300,display:"flex",alignItems:"flex-end"}}>
      <div style={{background:"#fff",borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"85vh",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
        <div style={{width:36,height:4,background:"#e5e7eb",borderRadius:2,margin:"12px auto 0"}}/>
        <div style={{padding:"12px 16px 10px",borderBottom:"1px solid #f1f0f5",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            <div style={{fontSize:15,fontWeight:900,color:"#1a0a4a"}}>⏳ Pending Logs</div>
            <div style={{fontSize:11,color:"#9b8ec4",marginTop:2}}>{pending.length} logs awaiting approval</div>
          </div>
          <button onClick={onClose} style={{width:28,height:28,borderRadius:"50%",background:"#f4f2ff",border:"none",fontSize:13,cursor:"pointer",color:"#5b3fc4"}}>✕</button>
        </div>
        <div style={{padding:"10px 16px 32px"}}>
          {loading?<div style={{padding:24,textAlign:"center",color:"#9b8ec4"}}>Loading...</div>
          :pending.length===0?<div style={{padding:32,textAlign:"center"}}>
            <div style={{fontSize:32,marginBottom:8}}>✅</div>
            <div style={{fontWeight:700,color:"#1a0a4a"}}>Sab approve ho gaya!</div>
          </div>
          :pending.map(log=>(
            <div key={log.id} style={{background:"#fefce8",border:"1.5px solid #fde68a",borderRadius:12,padding:"12px",marginBottom:10}}>
              {rejectId===log.id?(
                <div>
                  <div style={{fontSize:12,fontWeight:800,color:"#1a0a4a",marginBottom:8}}>Reject reason (optional):</div>
                  <textarea value={reason} onChange={e=>setReason(e.target.value)} style={{width:"100%",padding:"8px 10px",border:"1.5px solid #e0d8ff",borderRadius:8,fontSize:12,fontFamily:"inherit",outline:"none",resize:"none",minHeight:60,boxSizing:"border-box"}} placeholder="Reason likhein..."/>
                  <div style={{display:"flex",gap:8,marginTop:8}}>
                    <button onClick={()=>{setRejectId(null);setReason("");}} style={{flex:1,padding:"8px",border:"1.5px solid #e0d8ff",borderRadius:9,background:"#fff",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>Cancel</button>
                    <button onClick={()=>reject(log.id)} disabled={processing===log.id} style={{flex:1,padding:"8px",border:"none",borderRadius:9,background:"#dc2626",color:"#fff",fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>Confirm Reject</button>
                  </div>
                </div>
              ):(
                <>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                    <div>
                      <div style={{fontSize:13,fontWeight:800,color:"#1a0a4a"}}>{log.client_name}</div>
                      <div style={{fontSize:11,color:"#9b8ec4",marginTop:2}}>{log.service} · {staffMap[log.staff_id]||"Staff"} · {new Date(log.date+"T00:00:00").toLocaleDateString("en-IN",{day:"numeric",month:"short"})}</div>
                    </div>
                    <div style={{fontSize:14,fontWeight:900,color:"#a16207"}}>₹{Number(log.amount||0).toLocaleString("en-IN")}</div>
                  </div>
                  <div style={{display:"flex",gap:8}}>
                    <button onClick={()=>approve(log.id)} disabled={processing===log.id} style={{flex:1,padding:"8px",background:"#e8fdf0",border:"1.5px solid #bbf7d0",borderRadius:9,color:"#16a34a",fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>✓ Approve</button>
                    <button onClick={()=>setRejectId(log.id)} disabled={processing===log.id} style={{flex:1,padding:"8px",background:"#fff0f0",border:"1.5px solid #fca5a5",borderRadius:9,color:"#dc2626",fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>✕ Reject</button>
                  </div>
                </>
              )}
            </div>
...
export default function StaffManagement({user,currentUser,showRevenue=true,onBack}){
  ...
  return(
    <div style={{height:"100%",display:"flex",flexDirection:"column",background:N.bg,fontFamily:"system-ui,-apple-system,sans-serif"}}>
      ...
      {/* Pending Banner */}
      {pendingCount>0&&(
        <div onClick={()=>setShowPending(true)} style={{background:"#fef9c3",borderBottom:"1.5px solid #fde68a",padding:"10px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:"pointer",flexShrink:0}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:16}}>⏳</span>
            <div>
              <div style={{fontSize:12,fontWeight:800,color:"#a16207"}}>{pendingCount} Pending Log{pendingCount>1?"s":""} — Approval Chahiye</div>
              <div style={{fontSize:10,color:"#a16207",marginTop:1}}>Tap karke approve/reject karo</div>
            </div>
          </div>
          <span style={{fontSize:14,color:"#a16207"}}>›</span>
        </div>
      )}
      ...
      {showPending&&<PendingModal salonId={salonId} staffMap={staffMap} onClose={()=>{setShowPending(false);loadData();}} onUpdate={()=>loadData()}/>}
    </div>
  );
}
