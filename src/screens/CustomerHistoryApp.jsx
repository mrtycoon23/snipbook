...
function WAPrompt({customer,visit,onSend,onSkip,salonName,salonId}){
  ...
  async function sendWithTemplate(tKey){
    if(!customer.phone){
      window.open(`https://wa.me/?text=${encodeURIComponent(`🙏 Namaste ${customer.name}!\n\n✂️ Visit Summary — ${visit.date}\n\nServices: ${(visit.services||[]).join(", ")}\n💰 ₹${visit.amount}\n\nThank you! 💈`)}`,"_blank");
    ...
    try{
      const res=await fetch("/api/send-summary",{
        ...
        body:JSON.stringify({
          ...
          templateType:tKey,
          visit:{date:visit.date,services:visit.services||[],amount:visit.amount,notes:visit.notes||"",photos:visit.photos||[]}
        })
      });
      if(res.ok){setStatus("sent");setTimeout(onSend,2000);}
      else{const d=await res.json().catch(()=>({}));setWaError(d?.error||`HTTP ${res.status}`);setStatus("error");}
    }catch(e){setWaError(e.message);setStatus("error");}
  }
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:800,display:"flex",alignItems:"flex-end"}}>
      <div style={{background:T.surface,borderRadius:"20px 20px 0 0",padding:"20px 18px 36px",width:"100%"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
          <div style={{width:44,height:44,borderRadius:14,background:T.purpleLight,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>💬</div>
          <div>
            <div style={{fontWeight:900,fontSize:15,color:T.text}}>Send Visit Summary?</div>
            <div style={{fontSize:12,color:T.ts,marginTop:2}}>to {customer.name}</div>
          </div>
        </div>
        <div style={{background:T.sub,border:`2px solid ${T.border}`,borderRadius:12,padding:12,marginBottom:14}}>
          ...
          {photoCount>0&&<div style={{fontSize:12,color:T.purple,fontWeight:700,marginTop:4}}>📸 {photoCount} photo{photoCount>1?"s":""} will also be included!</div>}
        </div>
        {status==="idle"&&<>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:12}}>
            {[
              ...
              {key:"visit_summary",label:"📋 Visit Summary",desc:"Service details + photos button"},
              {key:"bill_summary",label:"💰 Bill + Summary",desc:"Service + bill amount"},
            ].map(t=>(
              <button key={t.key} onClick={()=>sendWithTemplate(t.key)} style={{width:"100%",padding:"11px 14px",background:T.wa,border:"none",borderRadius:12,color:"#fff",fontFamily:"inherit",fontSize:13,fontWeight:800,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                <span>{t.label}</span><span style={{fontSize:11,opacity:0.85}}>{t.desc}</span>
              </button>
            ))}
          </div>
          <button onClick={onSkip} style={{width:"100%",padding:11,border:`2px solid ${T.border}`,borderRadius:12,background:T.surface,fontFamily:"inherit",fontSize:13,fontWeight:700,cursor:"pointer",color:T.tm}}>Skip</button>
        </>}
        {status==="sending"&&<div style={{background:"#ede9fe",borderRadius:12,padding:14,textAlign:"center",fontWeight:800,color:"#5b3fc4"}}>📤 Sending...</div>}
        {status==="sent"&&<div style={{background:T.gl,border:`2px solid ${T.gm}`,borderRadius:12,padding:14,textAlign:"center",fontWeight:800,color:T.gd}}>✅ Message sent on WhatsApp!</div>}
        {status==="error"&&<div style={{display:"flex",flexDirection:"column",gap:8}}>
          <div style={{background:T.red,border:`2px solid ${T.rb}`,borderRadius:12,padding:10,textAlign:"center",fontSize:12,color:T.rt,fontWeight:700}}>⚠️ Could not send{waError?<div style={{fontSize:10,marginTop:4}}>{waError}</div>:null}</div>
          <div style={{display:"flex",gap:10}}>
            ...
            <button onClick={()=>setStatus("idle")} style={{flex:1,padding:12,background:T.wa,border:"none",borderRadius:12,color:"#fff",fontFamily:"inherit",fontSize:13,fontWeight:800,cursor:"pointer"}}>🔄 Retry</button>
          </div>
        </div>}
      </div>
    </div>
...
function AllVisitsModal({visits,staffMap,label,onClose}){
  const sorted=[...visits].sort((a,b)=>{const da=parseVisitDate(a.date),db=parseVisitDate(b.date);if(!da||!db)return 0;return db-da;});
      ...
      <div style={{background:"#fff",borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"85vh",display:"flex",flexDirection:"column"}}>
        <div style={{padding:"16px 18px 12px",borderBottom:`0.5px solid ${T.border}`,flexShrink:0}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div><div style={{fontWeight:900,fontSize:16,color:T.text}}>All Services</div><div style={{fontSize:11,color:T.ts,marginTop:2}}>{label} · {sorted.length} entries · ₹{total.toLocaleString("en-IN")}</div></div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"6px 18px 24px"}}>
          {sorted.length===0?<div style={{textAlign:"center",color:T.tf,padding:"32px 0",fontSize:13}}>No entries for this period</div>:sorted.map((v,i)=>(
  ...
  return(
    <div style={{flex:1,overflowY:"auto",padding:"0 0 80px",background:N.bg}}>
        ...
        {showCustom&&(
          <div style={{marginTop:10}}>
            <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:8}}>
              <input type="date" value={customFrom} onChange={e=>setCustomFrom(e.target.value)} style={{flex:1,padding:"9px 12px",border:`1px solid ${N.border}`,borderRadius:10,fontSize:13,fontFamily:"inherit",outline:"none",background:N.white,color:N.text}}/>
              <span style={{color:N.muted,fontSize:14,fontWeight:500}}>→</span>
              <input type="date" value={customTo} onChange={e=>setCustomTo(e.target.value)} style={{flex:1,padding:"9px 12px",border:`1px solid ${N.border}`,borderRadius:10,fontSize:13,fontFamily:"inherit",outline:"none",background:N.white,color:N.text}}/>
            </div>
            <button onClick={()=>{if(canApply){setAppliedFrom(customFrom);setAppliedTo(customTo);setPreset("custom");setShowCustom(false);}}} style={{width:"100%",padding:"10px",background:canApply?"#5b3fc4":"#d1d5db",border:"none",borderRadius:10,color:"#fff",fontFamily:"inherit",fontSize:13,fontWeight:700,cursor:canApply?"pointer":"not-allowed"}}>
      ...
      <div style={{margin:"12px 16px 0",background:"linear-gradient(135deg,#3d2490,#5b3fc4)",borderRadius:20,padding:"18px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{fontSize:32,fontWeight:800,color:"#fff",letterSpacing:"-0.5px",marginBottom:4}}>₹{totalRevenue>=1000?(totalRevenue/1000).toFixed(1)+"k":totalRevenue.toLocaleString()}</div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,margin:"12px 16px 0"}}>
        {[{icon:"💰",val:totalRevenue>=1000?`₹${(totalRevenue/1000).toFixed(1)}k`:`₹${totalRevenue}`,label:"Revenue",color:"#5b3fc4",ibg:"#ede9fe"},{icon:"✂️",val:totalVisits,label:"Total Visits",color:"#dc2626",ibg:"#fee2e2"},{icon:"👥",val:uniqueCustomers,label:"Customers",color:"#2563eb",ibg:"#dbeafe"},{icon:"📊",val:`₹${avgTicket}`,label:"Avg. Ticket Size",color:"#d97706",ibg:"#fef3c7"}].map(s=>(
          <div key={s.label} style={{background:N.white,borderRadius:14,padding:"14px",border:`1px solid ${N.border}`,display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1,minWidth:0}}><div style={{fontSize:18,fontWeight:800,color:s.color,lineHeight:1}}>{s.val}</div><div style={{fontSize:10,color:N.muted,marginTop:3,fontWeight:500}}>{s.label}</div></div>
          </div>
        ))}
      </div>
        ...
        <div style={{background:N.white,borderRadius:14,margin:"12px 16px 0",border:`1px solid ${N.border}`,padding:"14px"}}>
          ...
          {topServices.map(([svc,count],i)=>(
            <div key={svc} style={{display:"flex",alignItems:"center",gap:12,marginBottom:i<topServices.length-1?14:0}}>
              <div style={{width:36,height:36,borderRadius:10,background:i===0?"#ede9fe":i===1?"#fee2e2":"#eff6ff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{i===0?"💰":i===1?"✂️":"👤"}</div>
              <div style={{flex:1}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><span style={{fontSize:13,fontWeight:600,color:N.text}}>{svc}</span><span style={{fontSize:11,color:N.muted}}>{Math.round((count/totalVisits)*100)||0}%</span></div>
                <div style={{height:5,borderRadius:20,background:"#f1f0f5",overflow:"hidden"}}><div style={{width:`${(count/topServices[0][1])*100}%`,height:"100%",background:i===0?"#5b3fc4":i===1?"#dc2626":"#2563eb",borderRadius:20}}/></div>
              </div>
      ...
      {totalVisits===0&&(<div style={{background:N.white,border:`1px dashed ${N.border}`,borderRadius:14,padding:"32px",textAlign:"center",margin:"12px 16px 0"}}><div style={{fontSize:32,marginBottom:8}}>📭</div><div style={{fontWeight:700,fontSize:14,color:N.muted}}>No visits in this period</div></div>)}
      {showAllVisits&&<AllVisitsModal visits={filteredVisits} staffMap={staffMap} label={label} onClose={()=>setShowAllVisits(false)}/>}
    </div>
  );
}

function VisitCard({visit,index,isStaff,onUpdate,onWA,staffMap}){
  ...
  const staffName=staffMap?.[visit.stylist]||visit.stylist||"—";
...
function CustomerDetail({customer,isStaff,currentUser,onBack,onUpdate,staffMap,salonName,salonId}){
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:T.bg}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px 14px 80px",background:T.bg}}>
        ...
        {tab==="visits"&&<>
          ...
          {(customer.visitHistory||[]).length===0?<div style={{background:"#fff",border:`0.5px dashed ${T.border}`,borderRadius:12,padding:32,textAlign:"center",color:T.tf}}>No visits recorded yet</div>:(customer.visitHistory||[]).map((v,i)=><VisitCard key={v.id} visit={v} index={i} isStaff={isStaff} onUpdate={hvUpdate} onWA={setWaVisit} staffMap={staffMap}/>)}
        </>}
        {tab==="photos"&&<>
          <SL>All Photos</SL>
          {(customer.visitHistory||[]).every(v=>!(v.photos||[]).length)?<div style={{background:"#fff",border:`0.5px dashed ${T.border}`,borderRadius:12,padding:32,textAlign:"center",color:T.tf}}>No photos yet.</div>:(customer.visitHistory||[]).map(v=>{if(!(v.photos||[]).length)return null;return(<div key={v.id} style={{marginBottom:16}}><div style={{fontSize:12,fontWeight:800,color:T.ts,marginBottom:8,display:"flex",justifyContent:"space-between"}}><span>{fmtDate(v.date)}</span><span style={{color:T.tf}}>{(v.services||[]).join(" + ")}</span></div><div style={{display:"flex",gap:10,flexWrap:"wrap"}}>{(v.photos||[]).map((ph,i)=>(<div key={i} style={{width:90,height:90,borderRadius:12,overflow:"hidden",border:`2px solid ${T.border}`}}><img src={ph.url} alt="visit" style={{width:"100%",height:"100%",objectFit:"cover"}}/></div>))}</div></div>);})}
        </>}
      </div>
      ...
      {waVisit&&<WAPrompt customer={customer} visit={waVisit} salonName={salonName} salonId={salonId} onSend={()=>setWaVisit(null)} onSkip={()=>setWaVisit(null)}/>}
      {waPrompt&&<WAPrompt customer={customer} visit={waPrompt} salonName={salonName} salonId={salonId} onSend={()=>setWaPrompt(null)} onSkip={()=>setWaPrompt(null)}/>}
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:N.bg}}>
      <div style={{background:N.white,padding:"14px 18px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:`1px solid ${N.border}`,flexShrink:0}}>
        <div><div style={{fontSize:20,fontWeight:800,color:N.text,letterSpacing:"-0.3px"}}>Clients</div><div style={{fontSize:11,color:N.muted,marginTop:2,fontWeight:500}}>Manage your customers</div></div>
      ...
      {!isStaff&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",background:N.white,borderBottom:`1px solid ${N.border}`,flexShrink:0}}>
        {[{label:"Total",val:customers.length,color:N.text},{label:"VIP",val:customers.filter(c=>c.tag==="VIP").length,color:"#d97706"},{label:"Regular",val:customers.filter(c=>c.tag==="Regular").length,color:N.mid},{label:"New",val:customers.filter(c=>c.tag==="New").length,color:"#16a34a"}].map(s=>(
            ...
            return(<div key={c.id} onClick={()=>onSelect(c)} style={{background:"#fff",borderRadius:16,padding:"14px",cursor:"pointer",border:"1px solid #f1f0f5",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
                ...
                <div style={{display:"flex",gap:12,alignItems:"center",flexShrink:0}}>
                  <div style={{textAlign:"center"}}><div style={{fontSize:14,fontWeight:800,color:"#0f0a2e"}}>{c.visits||0}</div><div style={{fontSize:9,color:"#6b7280",marginTop:1}}>Visits</div></div>
                  {!isStaff&&<div style={{textAlign:"center"}}><div style={{fontSize:14,fontWeight:800,color:"#5b3fc4"}}>₹{((c.total_spent||c.totalSpent||0)/1000).toFixed(1)}k</div><div style={{fontSize:9,color:"#6b7280",marginTop:1}}>Spent</div></div>}
...
export default function CustomerHistory({currentUser,onBack,initialCustomerId}){
  ...
  async function loadData(){
    ...
    const{data:sData}=await supabase.from("staff").select("id,name").eq("salon_id",salonId);
    const{data:salData}=await supabase.from("salons").select("salon_name").eq("id",salonId).single();
    if(salData?.salon_name)setSalonName(salData.salon_name);
    if(sData){const map={};sData.forEach(s=>{map[s.id]=s.name;});setStaffMap(map);}
    if(cData&&cData.length>0){
      const grouped={};
      if(vData){vData.forEach(v=>{if(!grouped[v.customer_id])grouped[v.customer_id]=[];grouped[v.customer_id].push({id:v.id,date:v.date,services:v.services||[],stylist:v.stylist,amount:v.amount,notes:v.notes||"",photos:Array.isArray(v.photos)?v.photos:[]});});}
      setCustomers(cData.map(c=>{
        const vh=grouped[c.id]||[];
        const totalSpent=vh.reduce((s,v)=>s+(v.amount||0),0);
        const lastVisit=vh.length>0?[...vh].sort((a,b)=>new Date(b.date)-new Date(a.date))[0].date:"-";
        const visits=vh.length;
        const svcCount={};vh.forEach(v=>(v.services||[]).forEach(s=>{svcCount[s]=(svcCount[s]||0)+1;}));
        const favServices=Object.entries(svcCount).sort((a,b)=>b[1]-a[1]).map(([s])=>s);
        return{...c,avatar:c.avatar||(c.name?.slice(0,2)||"??").toUpperCase(),color:c.color||T.purple,visitHistory:vh,totalSpent,visits,lastVisit,favServices,tag:c.tag||"New",src:c.source||"wa",email:c.email||""};
      }));
    }else{setCustomers([]);}
    setLoading(false);
  }
    ...
    <div style={{height:"100%",display:"flex",flexDirection:"column",fontFamily:"system-ui,-apple-system,sans-serif",color:T.text,background:T.bg,overflow:"hidden"}}>
      {!selected&&isOwner&&<div style={{background:"#fff",padding:"14px 18px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid #f1f0f5",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div><div style={{fontWeight:800,fontSize:16,color:"#0f0a2e",letterSpacing:"-0.3px"}}>Customer History</div><div style={{fontSize:11,color:"#9b8ec4",marginTop:2,fontWeight:500}}>Visit records & analytics</div></div>
        </div>
      </div>}
      {isOwner&&!selected&&(
      ...
      {selected?<CustomerDetail customer={selected} isStaff={isStaff} currentUser={currentUser} onBack={()=>setSelectedId(null)} onUpdate={handleUpdate} staffMap={staffMap} salonName={salonName} salonId={salonId}/>:isOwner&&ownerTab==="dashboard"?<OwnerDashboard customers={customers} staffMap={staffMap}/>:<CustomerList customers={customers} isStaff={isStaff} onSelect={c=>setSelectedId(c.id)} onAddCustomer={()=>setShowAddCustomer(true)}/>}
