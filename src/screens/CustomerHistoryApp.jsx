...
export default function CustomerHistory({currentUser,onBack}){
  ...
  async function loadData(){setLoading(true);const salonId=currentUser?.salon_id||currentUser?.id;const{data:cData}=await supabase.from("customers").select("*").eq("salon_id",salonId);const{data:vData}=await supabase.from("visit_history").select("*").eq("salon_id",salonId);const{data:sData}=await supabase.from("staff").select("id,name").eq("salon_id",salonId);if(sData){const map={};sData.forEach(s=>{map[s.id]=s.name;});setStaffMap(map);}if(cData&&cData.length>0){const grouped={};if(vData){vData.forEach(v=>{if(!grouped[v.customer_id])grouped[v.customer_id]=[];const rawDate=v.date||"";const fmtDate=rawDate.includes("-")&&rawDate.length===10?new Date(rawDate+"T00:00:00").toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}):rawDate;grouped[v.customer_id].push({id:v.id,date:fmtDate,services:v.services||[],stylist:v.stylist,amount:v.amount,notes:v.notes||"",photos:Array.isArray(v.photos)?v.photos:[]});});}setCustomers(cData.map(c=>{const vh=grouped[c.id]||[];const totalSpent=vh.reduce((s,v)=>s+(v.amount||0),0);const lastVisit=vh.length>0?vh.sort((a,b)=>new Date(b.date)-new Date(a.date))[0].date:"-";const visits=vh.length;const svcCount={};vh.forEach(v=>(v.services||[]).forEach(s=>{svcCount[s]=(svcCount[s]||0)+1;}));const favServices=Object.entries(svcCount).sort((a,b)=>b[1]-a[1]).map(([s])=>s);return{...c,avatar:c.avatar||(c.name?.slice(0,2)||"??").toUpperCase(),color:c.color||T.purple,visitHistory:vh,totalSpent,visits,lastVisit,favServices,tag:c.tag||"New",src:c.source||"wa",email:c.email||""};}));}else{setCustomers([]);}setLoading(false);}
