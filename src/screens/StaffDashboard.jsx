    ...
    try{
      if(salonId){
        ...
        if(!existingCustomers||existingCustomers.length===0){
          setPendingLogData(base);setSaving(false);setShowNewCustomer(true);return;
        }
        if(existingCustomers.length>1){
          setAmbiguousCustomers(existingCustomers);setPendingLogData(base);setSaving(false);setShowPickCustomer(true);return;
        }
    ...
    return(
      <div onClick={e=>e.target===e.currentTarget&&handleCancel()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:200,display:"flex",alignItems:"flex-end"}}>
        <div style={{background:T.surface,borderRadius:"20px 20px 0 0",padding:"20px 18px 36px",width:"100%",maxHeight:"85vh",overflowY:"auto"}}>
          ...
          <button onClick={()=>{setShowPickCustomer(false);setShowNewCustomer(true);}} style={{width:"100%",padding:13,border:`2px solid ${T.green}`,borderRadius:12,background:T.gl,color:T.gd,fontFamily:"inherit",fontSize:14,fontWeight:800,cursor:"pointer",marginBottom:8}}>➕ This is a New Customer</button>
    ...
    return(
      <div onClick={e=>e.target===e.currentTarget&&handleCancel()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:200,display:"flex",alignItems:"flex-end"}}>
        <div style={{background:T.surface,borderRadius:"20px 20px 0 0",padding:"20px 18px 36px",width:"100%",maxHeight:"90vh",overflowY:"auto"}}>
          ...
          <div style={{background:T.gl,border:`1.5px solid ${T.gm}`,borderRadius:10,padding:"10px 13px",marginBottom:16,fontSize:12,color:T.gd,fontWeight:700}}>💡 Save their details — it will also appear in the owner's dashboard!</div>
          ...
          <div style={{marginBottom:12}}>
            ...
            {!newCustDob&&<div style={{fontSize:10,color:T.tf,marginTop:3}}>Tap to select date of birth</div>}
          </div>
      ...
      <div style={{background:T.surface,borderRadius:"20px 20px 0 0",padding:"22px 18px 36px",width:"100%"}}>
        ...
        {waStatus==="idle"&&<>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:12}}>
            {[{key:"thankyou",label:"💬 Thank You",desc:"Simple thank you message"},{key:"visit_summary",label:"📋 Visit Summary",desc:"Service details summary"},{key:"bill_summary",label:"💰 Bill + Summary",desc:"Service + bill amount"}].map(t=>(
    ...
    <div onClick={e=>e.target===e.currentTarget&&handleCancel()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:200,display:"flex",alignItems:"flex-end"}}>
      <div style={{background:T.surface,borderRadius:"20px 20px 0 0",padding:"20px 18px 36px",width:"100%",maxHeight:"80vh",overflowY:"auto",overflowX:"hidden",boxSizing:"border-box",WebkitOverflowScrolling:"touch"}}>
        ...
        <div style={{marginBottom:12,position:"relative"}}>
          ...
          <input style={IS} placeholder="e.g. Anjali Mehta" value={clientName} onChange={e=>{setClientName(e.target.value);setSuppressSuggest(false);}} onFocus={e=>e.target.style.borderColor=T.green} onBlur={e=>e.target.style.borderColor=T.border} autoFocus/>
          {suggestions.length>0&&(
            <div style={{position:"absolute",top:"100%",left:0,right:0,zIndex:50,background:"#fff",border:`2px solid ${T.border}`,borderRadius:12,marginTop:4,boxShadow:"0 8px 24px rgba(0,0,0,0.12)",overflow:"hidden"}}>
              <div style={{fontSize:10,fontWeight:800,color:T.tf,padding:"7px 12px 4px",borderBottom:`1px solid ${T.border}`}}>EXISTING CUSTOMERS</div>
              {suggestions.map(c=>(
                <div key={c.id} onClick={()=>pickSuggestion(c)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 12px",borderBottom:"1px solid #f6f8fa",cursor:"pointer"}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,minWidth:0}}>
                    <div style={{width:28,height:28,borderRadius:9,background:"#ede9fe",color:"#5b3fc4",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,flexShrink:0}}>{(c.name||"?").slice(0,2).toUpperCase()}</div>
                    <div style={{fontSize:13,fontWeight:700,color:T.text,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{c.name}</div>
                  </div>
                  <div style={{fontSize:11,color:T.ts,fontWeight:600,flexShrink:0,marginLeft:8}}>{c.phone?`📱 ${(c.phone||"").replace(/\D/g,"").slice(-10)}`:"No phone"}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{marginBottom:12}}>
          ...
          <input style={{...IS,...(phoneConflict?{borderColor:"#f59e0b"}:{})}} type="tel" placeholder="e.g. 9876543210" value={clientPhone} onChange={e=>{setClientPhone(e.target.value.replace(/\D/g,"").slice(0,10));setSuppressSuggest(false);}} onFocus={e=>e.target.style.borderColor=phoneConflict?"#f59e0b":T.green} onBlur={e=>e.target.style.borderColor=phoneConflict?"#f59e0b":T.border}/>
          {phoneConflict
            ?<div style={{background:T.yellow,border:`1.5px solid ${T.yb}`,borderRadius:10,padding:"9px 12px",marginTop:6}}>
              <div style={{fontSize:12,color:T.yt,fontWeight:800,marginBottom:6}}>⚠️ This number is already saved as "{phoneConflict.name}"</div>
              <div style={{fontSize:11,color:T.yt,marginBottom:8}}>Saving with a different name will link this log to {phoneConflict.name}'s history.</div>
              <button onClick={()=>{setSuppressSuggest(true);setClientName(phoneConflict.name);setPhoneConflict(null);setSuggestions([]);}} style={{padding:"7px 12px",background:"#fff",border:`1.5px solid ${T.yb}`,borderRadius:9,fontSize:12,fontWeight:800,color:T.yt,cursor:"pointer",fontFamily:"inherit"}}>✓ Use "{phoneConflict.name}" instead</button>
            </div>
        ...
        <div style={{marginBottom:14}}>
          ...
          <textarea style={{...IS,resize:"vertical",lineHeight:1.6,minHeight:64,fontFamily:"inherit"}} placeholder="e.g. Shampooed and conditioned hair..." value={notes} onChange={e=>setNotes(e.target.value)} onFocus={e=>e.target.style.borderColor=T.green} onBlur={e=>e.target.style.borderColor=T.border}/>
        </div>
        <div style={{marginBottom:18}}>
...
function EntryDetailModal({log,onClose}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"flex-end",zIndex:999}} onClick={onClose}>
      <div style={{background:"#fff",borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"85vh",overflowY:"auto",WebkitOverflowScrolling:"touch",paddingBottom:"calc(28px + env(safe-area-inset-bottom,0px))"}} onClick={e=>e.stopPropagation()}>
...
function AttendanceTab({staff,logs,setLogs,attendance,setAttendance,showRevenue,absentNotes,setAbsentNotes,salonId,salonName,setSelectedLog}){
  ...
  const mPendingLogs=monthLogs.filter(l=>l.status==="pending");
      ...
      {streak>=3&&(
        <div style={{background:"linear-gradient(135deg,#fff7ed,#fef3c7)",border:"1px solid #fde68a",borderRadius:14,padding:"10px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div><div style={{fontSize:13,fontWeight:800,color:"#78350f"}}>{streak} Day Streak!</div><div style={{fontSize:10,color:"#a16207",marginTop:1}}>Keep it up!</div></div>
          </div>
...
export default function StaffDashboard({staff,showRevenue=false,onLogout}){
  ...
  if(loading){return(<div style={{height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:T.bg,fontFamily:"system-ui,sans-serif"}}><div style={{textAlign:"center"}}><div style={{fontSize:32,marginBottom:12}}>✂️</div><div style={{fontSize:14,color:T.ts,fontWeight:700}}>Loading...</div></div></div>);}
...
export function StaffLoginPage({salonId,onLogin,onBack}){
  ...
  function handleLogin(){
    ...
    setTimeout(()=>{
      ...
      else{setError("PIN galat hai! Dobara try karo.");setPin("");setLoading(false);}
    },800);
  }
