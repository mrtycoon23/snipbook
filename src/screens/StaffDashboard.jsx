...
function AttendanceTab({staff,logs,setLogs,attendance,setAttendance,showRevenue,absentNotes,setAbsentNotes,salonId}){
  ...
  return(
    <div style={{padding:"12px 14px 80px",background:N.bg}}>
      {/* White Hero Card */}
      <div style={{background:N.white,borderRadius:18,padding:"14px",border:"1px solid #e5e7eb",boxShadow:"0 2px 12px rgba(0,0,0,0.05)",marginBottom:10}}>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,flexShrink:0,width:90}}>
            <svg width="74" height="74" viewBox="0 0 74 74">
              ...
              <circle cx="37" cy="37" r="27" fill="none" stroke="#f87171" strokeWidth="9"
                ...
                strokeLinecap="round" transform="rotate(-90 37 37)"/>
              <text x="37" y="33" textAnchor="middle" fill={N.text} fontSize="10" fontWeight="800" fontFamily="system-ui">{monthPresent}/{monthAbsent}</text>
          ...
          <div style={{flex:1,display:"flex",flexDirection:"column",gap:7}}>
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>
                {heatDays.map((d,i)=>(<div key={i} style={{height:15,borderRadius:4,background:d.future?"#f1f0f5":d.present?"#bbf7d0":"#fca5a5"}}/>))}
              </div>
            </div>
...
function EntryDetailModal({log,onClose}){
  ...
  const si=svcIcon(log.service);
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:999,padding:"0 0 0"}} onClick={onClose}>
      <div style={{background:"#fff",borderRadius:"22px 22px 0 0",width:"100%",maxWidth:480,padding:"0 0 32px"}} onClick={e=>e.stopPropagation()}>
        {/* Handle */}
        <div style={{width:36,height:4,background:"#e5e7eb",borderRadius:2,margin:"12px auto 0"}}/>
        {/* Header */}
        <div style={{background:"linear-gradient(135deg,#3d2490,#5b3fc4)",padding:"16px 18px 14px",margin:"12px 0 0"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div>
              <div style={{fontSize:11,color:"rgba(255,255,255,0.6)"}}>Work Entry</div>
              <div style={{fontSize:16,fontWeight:800,color:"#fff",marginTop:2}}>Service Details</div>
            </div>
            <div onClick={onClose} style={{width:32,height:32,borderRadius:"50%",background:"rgba(255,255,255,0.15)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:14,color:"#fff"}}>✕</div>
          </div>
        </div>
        {/* Content */}
        <div style={{padding:"16px 18px"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14,padding:"12px",background:"#f8f7ff",borderRadius:12}}>
            <div style={{width:44,height:44,borderRadius:14,background:"linear-gradient(135deg,#5b3fc4,#2d1b69)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:800,color:"#fff",flexShrink:0}}>{(log.clientName||"?").slice(0,2).toUpperCase()}</div>
            <div>
        ...
        <div>
          {filtered.length===0
            ...
            :filtered.map((log,i)=>{
              ...
              return(
                <div key={log.id} onClick={()=>setSelectedLog(log)} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderBottom:"1px solid #f9f9f9",cursor:"pointer"}}>
                  <div style={{width:32,height:32,borderRadius:9,background:svcIconHelper(log.service).bg,border:`1px solid ${svcIconHelper(log.service).border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,flexShrink:0}}>{svcIconHelper(log.service).icon}</div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,fontWeight:700,color:N.text}}>{log.clientName}</div>
                    <div style={{fontSize:10,color:svcIconHelper(log.service).color,fontWeight:600,marginTop:1}}>{log.service} · {fd(log.date)}</div>
                  </div>
                  {showRevenue&&<div style={{fontSize:12,fontWeight:700,color:"#16a34a",flexShrink:0}}>{fc(log.amount)}</div>}
                  <span style={{color:"#9ca3af",fontSize:14,flexShrink:0}}>›</span>
                </div>
