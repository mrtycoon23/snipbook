...
// ─── Pending Logs Modal ────────────────────────────────────────────────────────
function PendingLogsModal({logs,onClose}){
  const pendingLogs = logs.filter(l=>l.status==="pending").sort((a,b)=>b.date.localeCompare(a.date));
  const totalPendingRev = pendingLogs.reduce((s,l)=>s+l.amount,0);
    ...
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:300,display:"flex",alignItems:"flex-end"}}>
      <div style={{background:"#fff",borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"85vh",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
        ...
        <div style={{padding:"14px 18px 10px",borderBottom:"1px solid #f1f0f5",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            <div style={{fontSize:15,fontWeight:900,color:T.text}}>⏳ Pending Approvals</div>
            <div style={{fontSize:11,color:T.ts,marginTop:2}}>{pendingLogs.length} logs · {fc(totalPendingRev)} under review</div>
          </div>
          <button onClick={onClose} style={{width:28,height:28,borderRadius:"50%",background:"#f1f0f5",border:"none",fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
        </div>
        <div style={{padding:"10px 16px 32px"}}>
          {pendingLogs.length===0
            ?<div style={{textAlign:"center",padding:"32px 0",color:T.ts,fontSize:13}}>Koi pending log nahi hai! 🎉</div>
            :pendingLogs.map((log,i)=>{
              const ic=svc(log.service);
              return(
                <div key={log.id||i} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 12px",background:"#fefce8",border:"1.5px solid #fde68a",borderRadius:12,marginBottom:8}}>
                  <div style={{width:36,height:36,borderRadius:10,background:ic.bg,border:`1.5px solid ${ic.bd}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{ic.icon}</div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,fontWeight:800,color:T.text}}>{log.clientName}</div>
                    <div style={{fontSize:11,color:ic.c,fontWeight:600,marginTop:1}}>{log.service} · {fd(log.date)}</div>
                  </div>
                  <div style={{fontSize:13,fontWeight:800,color:"#a16207",flexShrink:0}}>{fc(log.amount)}</div>
                </div>
              );
            })
          }
...
// ─── Staff Stats Modal ─────────────────────────────────────────────────────────
function StaffStatsModal({staff,logs,onClose}){
  const myLogs = logs.filter(l=>l.staffId===staff.id);
  const weekLogs = myLogs.filter(l=>l.date>=thisWeekStart&&(l.status==="approved"||!l.status));
  const monthLogs = myLogs.filter(l=>l.date>=thisMonthStart&&(l.status==="approved"||!l.status));
  const allLogs = myLogs.filter(l=>l.status==="approved"||!l.status);
  const weekRev = weekLogs.reduce((s,l)=>s+l.amount,0);
  const monthRev = monthLogs.reduce((s,l)=>s+l.amount,0);
  const totalRev = allLogs.reduce((s,l)=>s+l.amount,0);
  const weekClients = new Set(weekLogs.map(l=>l.clientName)).size;
  const monthClients = new Set(monthLogs.map(l=>l.clientName)).size;
  const topServices = Object.entries(myLogs.reduce((acc,l)=>{acc[l.service]=(acc[l.service]||0)+1;return acc;},{})).sort((a,b)=>b[1]-a[1]).slice(0,3);
  ...
  return(
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:300,display:"flex",alignItems:"flex-end"}}>
      <div style={{background:"#fff",borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"90vh",overflowY:"auto",WebkitOverflowScrolling:"touch"}}>
        <div style={{width:36,height:4,background:"#e5e7eb",borderRadius:2,margin:"12px auto 0"}}/>
        <div style={{background:"linear-gradient(135deg,#3d2490,#5b3fc4)",padding:"16px 18px 18px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            <div style={{fontSize:15,fontWeight:900,color:"#fff"}}>📊 My Performance</div>
            <button onClick={onClose} style={{width:28,height:28,borderRadius:"50%",background:"rgba(255,255,255,0.2)",border:"none",fontSize:13,cursor:"pointer",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:44,height:44,borderRadius:14,background:c.bg,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:16,color:c.text}}>{initials(staff?.name||"ST")}</div>
            <div>
              <div style={{fontSize:16,fontWeight:900,color:"#fff"}}>{staff?.name}</div>
              <div style={{fontSize:11,color:"rgba(255,255,255,0.7)",marginTop:2}}>{staff?.role}</div>
            </div>
          </div>
        </div>
        <div style={{padding:"16px 18px 32px"}}>
          ...
          <div style={{marginBottom:16}}>
            <div style={{fontSize:12,fontWeight:800,color:T.ts,marginBottom:8,letterSpacing:"0.5px"}}>THIS WEEK</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
              {[{label:"Revenue",value:fc(weekRev),bg:"#f0fdf4",c:"#16a34a"},{label:"Logs",value:weekLogs.length,bg:"#f5f3ff",c:"#5b3fc4"},{label:"Clients",value:weekClients,bg:"#eff6ff",c:"#2563eb"}].map(s=>(
                <div key={s.label} style={{background:s.bg,borderRadius:12,padding:"12px 10px",textAlign:"center"}}>
                  <div style={{fontSize:15,fontWeight:900,color:s.c,lineHeight:1}}>{s.value}</div>
                  <div style={{fontSize:10,color:T.ts,marginTop:4}}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
          {/* This Month */}
          <div style={{marginBottom:16}}>
            <div style={{fontSize:12,fontWeight:800,color:T.ts,marginBottom:8,letterSpacing:"0.5px"}}>THIS MONTH</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
              {[{label:"Revenue",value:fc(monthRev),bg:"#f0fdf4",c:"#16a34a"},{label:"Logs",value:monthLogs.length,bg:"#f5f3ff",c:"#5b3fc4"},{label:"Clients",value:monthClients,bg:"#eff6ff",c:"#2563eb"}].map(s=>(
                <div key={s.label} style={{background:s.bg,borderRadius:12,padding:"12px 10px",textAlign:"center"}}>
                  <div style={{fontSize:15,fontWeight:900,color:s.c,lineHeight:1}}>{s.value}</div>
                  <div style={{fontSize:10,color:T.ts,marginTop:4}}>{s.label}</div>
                </div>
          ...
          {/* All Time */}
          <div style={{marginBottom:16}}>
            <div style={{fontSize:12,fontWeight:800,color:T.ts,marginBottom:8,letterSpacing:"0.5px"}}>ALL TIME</div>
            <div style={{background:"linear-gradient(135deg,#f5f3ff,#ede9fe)",border:"1.5px solid #ddd6fe",borderRadius:12,padding:"14px 16px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div><div style={{fontSize:22,fontWeight:900,color:"#5b3fc4"}}>{fc(totalRev)}</div><div style={{fontSize:11,color:T.ts,marginTop:2}}>Total Revenue · {allLogs.length} logs</div></div>
          ...
          {topServices.length>0&&(
            <div>
              <div style={{fontSize:12,fontWeight:800,color:T.ts,marginBottom:8,letterSpacing:"0.5px"}}>TOP SERVICES</div>
              {topServices.map(([svc,cnt],i)=>(
                <div key={svc} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"9px 12px",background:i===0?"#fef9c3":T.sub,borderRadius:10,marginBottom:6,border:`1px solid ${i===0?"#fde68a":T.border}`}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <span style={{fontSize:14}}>{i===0?"🥇":i===1?"🥈":"🥉"}</span>
                    <span style={{fontSize:13,fontWeight:700,color:T.text}}>{svc}</span>
                  </div>
                  <span style={{fontSize:12,fontWeight:800,color:"#5b3fc4"}}>{cnt}x</span>
                </div>
              ))}
            </div>
          )}
...
function AddLogModal({staffId,salonId,salonName,isPresent,onSave,onClose}){
  ...
  function pickSuggestion(c){
    ...
    if(c.phone)setClientPhone((c.phone||"").replace(/\D/g,"").slice(-10));
    setSuggestions([]);
  }
      ...
      <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:200,display:"flex",alignItems:"flex-end"}}>
        <div style={{background:T.surface,borderRadius:"20px 20px 0 0",padding:"20px 18px 36px",width:"100%"}}>
          ...
          <div style={{textAlign:"center",padding:"20px 0"}}>
            ...
            <div style={{fontSize:13,color:T.ts,marginBottom:24}}>Attendance mark karo pehle.</div>
            <button onClick={onClose} style={{padding:"12px 32px",background:T.green,border:"none",borderRadius:12,color:"#fff",fontFamily:"inherit",fontSize:14,fontWeight:800,cursor:"pointer"}}>Got It</button>
          </div>
        </div>
      </div>
    );
    ...
    try{
      if(salonId){
        ...
        if(phone10.length===10){
          ...
          if(byPhone&&byPhone.length>0){await saveLog({...base,customerId:byPhone[0].id});}
          else{const{data:newCust}=await supabase.from("customers").insert({salon_id:salonId,name:base.clientName,phone:phone10,gender:newClientGender||"male",birthday:newClientDob||null}).select().single();await saveLog({...base,customerId:newCust?.id||null});}
          return;
        }
        ...
        if(!existingCustomers||existingCustomers.length===0){setPendingLogData(base);setSaving(false);setShowNewCustomer(true);return;}
        if(existingCustomers.length>1){setAmbiguousCustomers(existingCustomers);setPendingLogData(base);setSaving(false);setShowPickCustomer(true);return;}
        await saveLog({...base,customerId:existingCustomers[0].id});
        return;
      }
      await saveLog(base);
    }catch(e){setPendingLogData(base);setSaving(false);setShowNewCustomer(true);}
  }
      ...
      if(salonId){
        ...
        if(customerId){
          await supabase.from("visit_history").insert({
            ...
            services:[logData.service],stylist:logData.staffId,
            amount:logData.amount,notes:logData.notes||"",photos:logData.photos||[]
          });
        }
        if(res){
  ...
  async function saveNewCustomer(){
    ...
    try{
      ...
      if(salonId){
        const{data:created}=await supabase.from("customers").insert({salon_id:salonId,name:pendingLogData.clientName,phone:newCustPhone||"",birthday:newCustDob||null,gender:newCustGender||"male"}).select().single();
        newCustomerId=created?.id||null;
      }
      await saveLog({...pendingLogData,customerId:newCustomerId});
    }catch(e){console.error(e);await saveLog(pendingLogData);}
    ...
    return(
      <div onClick={e=>e.target===e.currentTarget&&handleCancel()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:200,display:"flex",alignItems:"flex-end"}}>
        <div style={{background:T.surface,borderRadius:"20px 20px 0 0",padding:"20px 18px 36px",width:"100%",maxHeight:"85vh",overflowY:"auto"}}>
          <div style={{width:36,height:4,background:T.border,borderRadius:2,margin:"0 auto 16px"}}/>
          <div style={{fontWeight:900,fontSize:15,color:T.text,marginBottom:14}}>🤔 "{pendingLogData.clientName}" — Which customer?</div>
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:14}}>
            {ambiguousCustomers.map(c=>(
              <button key={c.id} disabled={saving} onClick={async()=>{setShowPickCustomer(false);setSaving(true);await saveLog({...pendingLogData,customerId:c.id});}} style={{textAlign:"left",padding:"12px 14px",border:`2px solid ${T.border}`,borderRadius:12,background:T.sub,cursor:"pointer",fontFamily:"inherit"}}>
                ...
                <div style={{fontSize:12,color:T.ts,marginTop:2}}>{c.phone?`📱 ${c.phone}`:"No phone"}</div>
              </button>
            ))}
          </div>
          <button onClick={()=>{setShowPickCustomer(false);setShowNewCustomer(true);}} style={{width:"100%",padding:13,border:`2px solid ${T.green}`,borderRadius:12,background:T.gl,color:T.gd,fontFamily:"inherit",fontSize:14,fontWeight:800,cursor:"pointer",marginBottom:8}}>➕ New Customer</button>
    ...
    return(
      <div onClick={e=>e.target===e.currentTarget&&handleCancel()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:200,display:"flex",alignItems:"flex-end"}}>
        <div style={{background:T.surface,borderRadius:"20px 20px 0 0",padding:"20px 18px 36px",width:"100%",maxHeight:"90vh",overflowY:"auto"}}>
          <div style={{width:36,height:4,background:T.border,borderRadius:2,margin:"0 auto 16px"}}/>
          <div style={{fontWeight:900,fontSize:16,color:T.text,marginBottom:16}}>🆕 New Customer!</div>
          ...
          <div style={{marginBottom:12}}>
            ...
            <input style={{...IS,minHeight:44}} type="date" value={newCustDob} onChange={e=>setNewCustDob(e.target.value)}/>
          </div>
          <div style={{marginBottom:16}}>
          ...
          <button onClick={saveNewCustomer} disabled={savingCustomer||!newCustPhone||newCustPhone.length<10} style={{width:"100%",padding:13,background:savingCustomer||!newCustPhone||newCustPhone.length<10?"#d1d5db":T.green,border:"none",borderRadius:12,color:"#fff",fontFamily:"inherit",fontSize:14,fontWeight:800,cursor:"pointer"}}>
      ...
      <div style={{background:T.surface,borderRadius:"20px 20px 0 0",padding:"20px 18px 36px",width:"100%",maxHeight:"80vh",overflowY:"auto",overflowX:"hidden",boxSizing:"border-box",WebkitOverflowScrolling:"touch"}}>
        ...
        <div style={{marginBottom:12,position:"relative"}}>
          ...
          {suggestions.length>0&&(
            <div style={{position:"absolute",top:"100%",left:0,right:0,zIndex:50,background:"#fff",border:`2px solid ${T.border}`,borderRadius:12,marginTop:4,boxShadow:"0 8px 24px rgba(0,0,0,0.12)",overflow:"hidden"}}>
              {suggestions.map(c=>(
                <div key={c.id} onClick={()=>pickSuggestion(c)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 12px",borderBottom:"1px solid #f6f8fa",cursor:"pointer"}}>
                  <div style={{fontSize:13,fontWeight:700,color:T.text}}>{c.name}</div>
                  <div style={{fontSize:11,color:T.ts}}>{c.phone?`📱 ${(c.phone||"").slice(-10)}`:"No phone"}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{marginBottom:12}}>
          <div style={{fontSize:12,fontWeight:800,color:T.tm,marginBottom:5}}>Phone <span style={{color:T.tf,fontWeight:600}}>(optional)</span></div>
          <input style={{...IS,...(phoneConflict?{borderColor:"#f59e0b"}:{})}} type="tel" placeholder="9876543210" value={clientPhone} onChange={e=>{setClientPhone(e.target.value.replace(/\D/g,"").slice(0,10));setSuppressSuggest(false);}}/>
          {phoneConflict&&<div style={{background:T.yellow,border:`1.5px solid ${T.yb}`,borderRadius:10,padding:"9px 12px",marginTop:6}}>
            <div style={{fontSize:12,color:T.yt,fontWeight:800,marginBottom:6}}>⚠️ Number saved as "{phoneConflict.name}"</div>
            <button onClick={()=>{setSuppressSuggest(true);setClientName(phoneConflict.name);setPhoneConflict(null);setSuggestions([]);}} style={{padding:"7px 12px",background:"#fff",border:`1.5px solid ${T.yb}`,borderRadius:9,fontSize:12,fontWeight:800,color:T.yt,cursor:"pointer",fontFamily:"inherit"}}>✓ Use "{phoneConflict.name}"</button>
          </div>}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          <div><div style={{fontSize:12,fontWeight:800,color:T.tm,marginBottom:5}}>Service</div><select style={{...IS,cursor:"pointer"}} value={service} onChange={e=>setService(e.target.value)}>{SERVICES.map(s=><option key={s}>{s}</option>)}</select></div>
          <div><div style={{fontSize:12,fontWeight:800,color:T.tm,marginBottom:5}}>Date</div><input style={IS} type="date" value={date} onChange={e=>setDate(e.target.value)}/></div>
        </div>
        ...
        {isNewClient&&clientPhone.length===10&&(
          <div style={{background:"#eff6ff",border:"1.5px solid #93c5fd",borderRadius:11,padding:"12px 13px",marginBottom:12}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
              ...
              <div style={{fontSize:12,fontWeight:800,color:"#2563eb"}}>New Client! Add details:</div>
            </div>
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <button onClick={save} disabled={saving} style={{flex:2,padding:12,border:"none",borderRadius:12,background:clientName.trim()&&amount?T.green:"#d1d5db",color:"#fff",fontFamily:"inherit",fontSize:13,fontWeight:800,cursor:"pointer"}}>{saving?"Saving...":"✓ Save"}</button>
        </div>
      </div>
    </div>
  );
...
function AttendanceTab({staff,logs,setLogs,attendance,setAttendance,showRevenue,absentNotes,setAbsentNotes,salonId,salonName,setSelectedLog}){
  ...
  const monthLogs=logs.filter(l=>l.staffId===staff.id&&l.date>=thisMonthStart);
  const mLogs=monthLogs.length;
  const mRev=monthLogs.filter(l=>l.status==="approved"||!l.status).reduce((s,l)=>s+l.amount,0);
  const mPendingLogs=logs.filter(l=>l.staffId===staff.id&&l.status==="pending");
    ...
    <div style={{padding:"12px 14px 80px",background:N.bg}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,flexShrink:0,width:90}}>
            <svg width="74" height="74" viewBox="0 0 74 74">
              ...
              <circle cx="37" cy="37" r="27" fill="none" stroke="#22c55e" strokeWidth="9"
                strokeDasharray={`${(monthPresent/Math.max(monthPresent+monthAbsent,1))*170} 170`}
                strokeLinecap="round" transform="rotate(-90 37 37)"/>
              <circle cx="37" cy="37" r="27" fill="none" stroke="#f87171" strokeWidth="9"
                strokeDasharray={`${(monthAbsent/Math.max(monthPresent+monthAbsent,1))*170} 170`}
                strokeDashoffset={`-${(monthPresent/Math.max(monthPresent+monthAbsent,1))*170}`}
                strokeLinecap="round" transform="rotate(-90 37 37)"/>
              <text x="37" y="33" textAnchor="middle" fill={N.t} fontSize="10" fontWeight="800" fontFamily="system-ui">{monthPresent}/{monthAbsent}</text>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4,width:"100%"}}>
              {[{bg:"#f0fdf4",c:"#16a34a",v:monthPresent,l:"Present"},{bg:"#fff5f5",c:"#dc2626",v:monthAbsent,l:"Absent"},{bg:"#eff6ff",c:"#2563eb",v:mClients,l:"Clients"},{bg:"#f5f3ff",c:N.p,v:mLogs,l:"Logs"}].map(s=>(
          ...
          <div style={{flex:1,display:"flex",flexDirection:"column",gap:7}}>
            ...
            {mPendingLogs.length>0&&(
              <div onClick={()=>setShowPendingModal(true)} style={{background:"#fef9c3",border:"1.5px solid #fde68a",borderRadius:9,padding:"7px 10px",display:"flex",alignItems:"center",gap:6,cursor:"pointer"}}>
                <span style={{fontSize:12}}>⏳</span>
                <div style={{fontSize:10,fontWeight:700,color:"#a16207",flex:1}}>{mPendingLogs.length} logs pending · {fc(mPendingRev)} under review</div>
      ...
      {!isPresent&&(
        <div style={{background:N.w,border:"1px solid #f1f0f5",borderRadius:12,padding:"12px 14px",marginBottom:10}}>
          <div style={{fontSize:11,fontWeight:700,color:"#6b7280",marginBottom:6}}>🔒 Reason (private):</div>
          <input style={{...IS,background:"#f8f7ff",borderColor:"#e0d8ff"}} placeholder="Add reason..." value={(absentNotes||{})[today]||""} onChange={async e=>{setAbsentNotes(prev=>({...prev,[today]:e.target.value}));if(salonId)await supabase.from("attendance").upsert({salon_id:salonId,staff_id:staff.id,date:today,is_present:false,absent_reason:e.target.value},{onConflict:"salon_id,staff_id,date"});}}/>
        </div>
      )}
        ...
        <div>
          {filtered.length===0
            ?<div style={{textAlign:"center",padding:"22px 16px"}}>
              <div style={{fontSize:32,margin:"0 auto 10px"}}>✂️</div>
              <div style={{fontSize:13,fontWeight:800,color:N.t}}>No entries yet</div>
            </div>
            :filtered.map((log,i)=>{
              const ic=svc(log.service);
              return(
                <div key={log.id} onClick={()=>setSelectedLog(log)} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderBottom:"1px solid #f9f9f9",cursor:"pointer"}}>
                  ...
                  {log.status==="pending"&&<span style={{fontSize:9,background:"#fef9c3",color:"#a16207",borderRadius:5,padding:"2px 5px",fontWeight:700}}>⏳</span>}
      ...
      {showPendingModal&&<PendingLogsModal logs={logs.filter(l=>l.staffId===staff.id)} onClose={()=>setShowPendingModal(false)}/>}
      {showStatsModal&&<StaffStatsModal staff={staff} logs={logs} onClose={()=>setShowStatsModal(false)}/>}
    </div>
  );
}
...
export default function StaffDashboard({staff,showRevenue=false,onLogout}){
  ...
  useEffect(()=>{
    async function loadData(){
      ...
      if(logsData){setLogs(logsData.map(l=>({id:l.id,staffId:l.staff_id,clientName:l.client_name,service:l.service,amount:l.amount,date:l.date,status:l.status||"approved"})));}
      const{data:attData}=await supabase.from("attendance").select("*").eq("salon_id",salonId).eq("staff_id",staff.id);
      if(attData){
        const attMap={};
        attData.forEach(row=>{if(!attMap[row.date])attMap[row.date]={};attMap[row.date][row.staff_id]=row.is_present;});
  ...
  if(loading){return(<div style={{height:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:T.bg}}><div style={{textAlign:"center"}}><div style={{fontSize:32,marginBottom:12}}>✂️</div><div style={{fontSize:14,color:T.ts,fontWeight:700}}>Loading...</div></div></div>);}
...
export function StaffLoginPage({salonId,onLogin,onBack}){
  ...
  function handleLogin(){
    ...
    setTimeout(()=>{
      ...
      if(staff&&staff.pin===pin){onLogin({...staff,salon_id:salonId});}
      else{setError("PIN galat hai!");setPin("");setLoading(false);}
    },800);
  }

  if(loadingStaff){return(<div style={{minHeight:"100vh",background:"linear-gradient(135deg,#1a1a2e,#16213e)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui,sans-serif"}}><div style={{textAlign:"center",color:"#fff"}}><div style={{fontSize:32,marginBottom:12}}>✂️</div><div style={{fontSize:14,color:"#a0a0c0"}}>Loading...</div></div></div>);}
  ...
  if(!salonId||staffList.length===0){
    return(
      <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#1a1a2e,#16213e)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 20px",fontFamily:"system-ui,sans-serif"}}>
        <div style={{background:"#fff",borderRadius:20,padding:"26px 22px",width:"100%",maxWidth:360}}>
          <div style={{fontWeight:900,fontSize:17,marginBottom:8}}>Koi staff nahi mila</div>
          <button onClick={onBack} style={{width:"100%",padding:"13px",background:T.dark,border:"none",borderRadius:12,color:"#fff",fontFamily:"inherit",fontSize:14,fontWeight:800,cursor:"pointer"}}>← Back</button>
        </div>
      </div>
    );
  }
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#1a1a2e,#16213e)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 20px",fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{background:"#fff",borderRadius:20,padding:"26px 22px",width:"100%",maxWidth:360,boxShadow:"0 16px 48px rgba(0,0,0,0.3)"}}>
        <div style={{fontWeight:900,fontSize:17,marginBottom:16}}>Staff Login</div>
        <div style={{marginBottom:14}}>
        ...
        <button onClick={handleLogin} disabled={loading} style={{width:"100%",padding:"13px",background:loading?"#86efac":T.green,border:"none",borderRadius:12,color:"#fff",fontFamily:"inherit",fontSize:15,fontWeight:800,cursor:"pointer",marginBottom:12}}>{loading?"Logging in...":"Login →"}</button>
        <button onClick={onBack} style={{width:"100%",background:"none",border:"none",color:T.ts,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>← Back</button>
      </div>
    </div>
  );
}
